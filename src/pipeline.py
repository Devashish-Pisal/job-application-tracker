...
def process_high_conf_output(sheet_service,llm_output, email_data, logging_obj):
    ...
    if company_name and role and company_name != "NULL" and role != "null": # Company name - Present; Role - Present
        count, matches = fuzzy_match_company_and_role(sheet_service, SHEET_ID, company_name_col_index, role_col_index, company_name, role, threshold=0.87)
        if count == 0:
        ...
        count, matches = fuzzy_match_column_values(sheet_service, SHEET_ID, company_name_col_index, company_name, company_fuzzy_threshold)
        if count == 0:
        ...
        count, matches = fuzzy_match_column_values(sheet_service, role_col_index, role, role_fuzzy_threshold)
        if count == 0:
            append_row(sheet_service, SHEET_ID, new_entry)
        elif count == 1:
            modify_row(sheet_service, SHEET_ID, matches[0]["row_index"], llm_output, email_data)
